...
const filterForecastData = (data, toggledDays = {}) => {
    ...
    Object.keys(data).forEach((date) => {
        if (toggledDays[date]) {
            ...
            const nextEntries = data[date].filter(
                (entry) => new Date(entry.dt * 1000).getHours() > currentHour
            );
    ...
    return (
        <div>
            <h2>
                ...
                {submittedLocation ? ` for ${submittedLocation}` : ""}
            </h2>
            <input
                type="text"
                ...
                <div>
                    <div className="card-container">
                        {Object.keys(forecast).map((date, index) => (
                            <div
                                className={`card ${
                                    toggledDays[date] ? "card-expanded" : ""
                                }`}
                                key={index}
                            >
                                <h3 className="card-date">{`${getDayOfWeek(
                                    forecast[date][0]?.dt
                                )} ${date.slice(0, 5)}`}</h3>
                                ...
                                <button
                                    onClick={() => handleToggleDayHourly(date)}
